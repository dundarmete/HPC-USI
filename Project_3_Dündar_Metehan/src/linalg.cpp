...
// x and y are vectors on length N
// alpha is a scalar
void hpc_axpy(Field& y, const double alpha, const Field& x, const int N)
{
    #pragma omp parallel for
    for (int i = 0; i < N; i++)
    {
...
// alpha is a scalar
void hpc_add_scaled_diff(Field& y, const Field& x, const double alpha,
                          const Field& l, const Field& r, const int N)
{
    #pragma omp parallel for
    for (int i = 0; i < N; i++)
    {
...
void hpc_scale(Field& y, const double alpha, const Field& x, const int N)
{
    #pragma omp parallel for
    for (int i = 0; i < N; ++i)
    {
...
void hpc_copy(Field& y, const Field& x, const int N)
{
    #pragma omp parallel for
    for (int i = 0; i < N; i++) {
        y[i] = x[i];
