...
class Field:
    ...
    def exchange_startall(self):
        ...
        # ... implement ...
        self._reqs = [MPI.REQUEST_NULL] * 4
        
        if domain.neighbour_north >= 0:
            self._buffN = np.array([self.inner[:, -1]])
            self._reqs.append(domain.comm_cart.Isend(
                [
                    self._buffN, MPI.DOUBLE
                ],
                dest=domain.neighbour_north)
            )
            self._reqs.append(domain.comm_cart.Irecv(
                [
    ...
    def exchange_waitall(self):
        ...
        # ... implement ...
        stats = [MPI.Status() for _ in range(len(self._reqs))]
        MPI.Request.Waitall(self._reqs, stats)
