...
int main(int argc, char *argv[])
{
    ...
    // To the top
    MPI_Sendrecv(&data[DOMAINSIZE + 1], SUBDOMAIN, MPI_DOUBLE, rank_top, 0,
                &data[(DOMAINSIZE-1) * DOMAINSIZE + 1], SUBDOMAIN, MPI_DOUBLE, rank_bottom, 0,
                comm_cart, &status);

    // To the bottom
    MPI_Sendrecv(&data[(DOMAINSIZE-2) * DOMAINSIZE + 1], SUBDOMAIN, MPI_DOUBLE, rank_bottom, 1,
                &data[1], SUBDOMAIN, MPI_DOUBLE, rank_top, 1,
                comm_cart, &status);

    // To the left
    MPI_Sendrecv(&data[DOMAINSIZE + 1], 1, data_ghost, rank_left, 2,
                &data[2 * DOMAINSIZE - 1], 1, data_ghost, rank_right, 2,
                comm_cart, &status);

    // To the right
    MPI_Sendrecv(&data[2 * DOMAINSIZE - 2], 1, data_ghost, rank_right, 3,
                &data[DOMAINSIZE], 1, data_ghost, rank_left, 3,
                comm_cart, &status);
